spConjNNGP <- function(formula, data = parent.frame(), coords, n.neighbors = 15,
    ...
    if(!missing(k.fold)){
        ...
        for(i in 1:k.fold){
            ...
            mod.id <- sort(sample(1:n, floor(length(y)/k.fold)))
            
            X.mod <- X[mod.id,,drop=FALSE]
            y.mod <- y[mod.id]
            coords.mod <- coords[mod.id,,drop=FALSE]
            n.mod <- nrow(X.mod)

            X.ho <- X[-mod.id,,drop=FALSE]
            y.ho <- y[-mod.id]
            coords.ho <- coords[-mod.id,,drop=FALSE]
            n.ho <- nrow(X.ho)
